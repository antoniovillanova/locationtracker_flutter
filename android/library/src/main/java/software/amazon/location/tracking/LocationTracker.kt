...
class LocationTracker {
    ...
    constructor(
        ...
        (locationCredentialsProvider.getCredentialsProvider() as? CognitoCachingCredentialsProvider)?.let {
            val strRegion = it.identityPoolId.split(":").first()
            amazonLocationClient.setRegion(com.amazonaws.regions.Region.getRegion(strRegion))
        }
        ...
        val locationsToUpload = allEntries.filter { location ->
            clientConfig.locationFilters.all { filter ->
                val isFilterEnabled = when (filter) {
                    is TimeLocationFilter -> awsKeyValueStore.get(IS_TIME_FILTER_ENABLE)
                        ?.toBoolean() ?: false

                    is DistanceLocationFilter -> awsKeyValueStore.get(IS_DISTANCE_FILTER_ENABLE)
                        ?.toBoolean() ?: false

                    is AccuracyLocationFilter -> awsKeyValueStore.get(IS_ACCURACY_FILTER_ENABLE)
                        ?.toBoolean() ?: false
    ...
    fun enableFilter(filter: LocationFilter) {
        when (filter) {
            ...
            is DistanceLocationFilter -> awsKeyValueStore.put(
                IS_DISTANCE_FILTER_ENABLE,
                true.toString()
            )

            is AccuracyLocationFilter -> awsKeyValueStore.put(
                IS_ACCURACY_FILTER_ENABLE,
                true.toString()
            )
        }
