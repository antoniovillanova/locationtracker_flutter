package com.nttdata.travelmar.viewModel
...
import com.nttdata.travelmar.utils.Constant.PREFS_NAME_TRACKING
import com.nttdata.travelmar.utils.Constant.SERVICE_NAME
import com.nttdata.travelmar.utils.SigV4Interceptor
...
class MainViewModel : ViewModel() {
    ...
    fun evaluateGeofence(
        ...
        actualTime.set(Calendar.MINUTE, actualTime.get(Calendar.MINUTE) - 2)

        locationEntry.forEach {
            val devicePositionUpdate =
                DevicePositionUpdate().withPosition(it.longitude, it.latitude)
                    .withDeviceId(deviceId)
                    .withSampleTime(actualTime.time as Date)
                    .withPositionProperties(map)
            devicePositionUpdateList.add(devicePositionUpdate)
        }

        val data = BatchUpdateDevicePositionRequest().withTrackerName(trackerName)
            .withUpdates(devicePositionUpdateList)
        ...
        return try {
            val awsResponse = amazonLocationClient?.batchUpdateDevicePosition(data)
            awsResponse
            true
        } catch (e: Exception) {
            e.printStackTrace()
